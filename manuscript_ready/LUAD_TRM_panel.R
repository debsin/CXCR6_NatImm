...
pdf(file.path("C:/Projects/CXCR6/manuscript_ready/TRM_LUAD_panel.pdf"), width = 5, height = 5)
pairs.panels(df.signatures[,c(1:3,6)],
             stars = T,
...
pairs.panels(df.signatures[,1:3],
             stars = T,
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             main = paste(full_names[1], paste0('(',c_type,')')),
...
pairs.panels(df.signatures[,c(1:3,4)],
             stars = T,
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             main = paste(full_names[1], paste0('(',c_type,')')),
...
pairs.panels(df.signatures[,c(1:3,4,6)],
             stars = T,
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             main = paste(full_names[1], paste0('(',c_type,')')),
...
pdf(file.path("C:/Projects/CXCR6/manuscript_ready/TRM_LUAD_panel_exclude_dermis.pdf"), width = 4, height = 4)
pairs.panels(df.signatures[,-5],
             stars = T,
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             main = paste(full_names[1], paste0('(',c_type,')')),
...
sp <- ggscatter(df.signature, x = "TRM", y = "CXCR6",size = 0.5,title  = full_names[c_type],
...
lay <- rbind(c(NA,NA,NA,2,2,2),
             c(1,1,1,2,2,2),
             c(1,1,1,2,2,2),
             c(1,1,1,3,3,3),
             c(1,1,1,3,3,3),
             c(NA,NA,NA,3,3,3))
grid.arrange(mar.plot,
             ggplotGrob(p[[1]]),
             ggplotGrob(p[[2]]), layout_matrix = lay)
