...
library(survival)
library(gridExtra)
c_types = c("LUAD","CESC","HNSC",
            "SKCM","BRCA")
full_names = c("Lung Adenocarcinoma", 
               "Cervical squamous cell cancer","Head and Neck Cancer",
               "Skin Cutaneous Melanoma","Breast Cancer" )

names(full_names)  = c_types

gene_symbols = NULL

pdf(file.path("C:/Projects/CXCR6/manuscript_ready/Fig9_G-J_CCR2_CXCR6.pdf"), width = 8, height = 4)

# c_type = c_types[10]
for(c_type in c_types){
  dir = file.path("C:/Projects/CXCR6/TCGA/", c_type)
  setwd(dir)
  ...
  plotSurvSig<-function(sigName = NULL){
    ...
    pg<-ggsurvplot(fit, data = df,pval.method = T,
  ...
  sp <- ggscatter(df.signature, x = "CCR2", y = "CXCR6",size = 0.5,
                  # title  = paste0(full_names[c_type], " (",c_type,")"),
                  add = "reg.line",  # Add regressin line
                  cor.coef = TRUE, cor.coeff.args = list(cor.coef.name = "rho", method = "spearman"),
                  add.params = list(color = "blue", fill = "lightgray"), # Customize reg. line
                  conf.int = TRUE # Add confidence interval
  )+ggtitle(label = paste0(full_names[c_type], " (",c_type,")"))+
    font("title", size = 12, face = "bold.italic")+
    xlab("CCR2 expression")+
    ylab("CXCR6 expression")+
    geom_text(x = max(df.signature$CCR2)-0.25, y = 0.25, size = 2,label = paste(length(intersect.ids), "patients"),check_overlap = T)
  
  # cor.plot = sp + stat_cor(method = "spearman", cor.coef.name = "rho")+ 
    
    
  
  mar.plot = ggMarginal(sp, type = "density",fill = "#00AFBB88")
  
  # mar.plot

  grid.arrange(mar.plot,ggplotGrob(p[[3]]) ,ncol = 2)
